...
<title>${projectName}-定时任务-任务管理</title>
		...
		<div class="c-right">
			<div class="panel panel-success">
				<div class="panel-heading panel-heading-tool">
					<div class="row">
						<div class="col-sm-5 title">任务管理 / <b>任务管理</b></div>
						<div class="col-sm-7 text-right">
							<div class="btn-group">
						  		<a href="${webroot}/ttsTaskProject/f-view/manager.shtml" class="btn btn-default btn-sm">返回</a>
...
var info = {
		...
		loadInfo : function(page) {
			...
			JUtil.ajax({
				url : '${webroot}/ttsTaskJob/f-json/pageQuery.shtml',
					...
					if(json.code === 0) {
						function getResult(obj) {
							...
							if(obj.status === 0 || obj.status === <%=JobStatus.WAIT.getCode()%>) {
								_operate.push(' &nbsp; <a href="javascript:info.status(',obj.id,',<%=JobStatus.STOP.getCode()%>)" class="glyphicon glyphicon-pause text-danger" title="暂停"></a>');
							} else {
								_operate.push(' &nbsp; <a href="javascript:info.status(',obj.id,',<%=JobStatus.NORMAL.getCode()%>)" class="glyphicon glyphicon-play text-success" title="启动"></a>');
							}
							_operate.push(' &nbsp; <a class="glyphicon glyphicon-edit text-success" href="javascript:info.edit(',obj.id,')" title="修改"></a>',
							    	...
							    	' &nbsp; <a class="glyphicon text-success" href="javascript:info.execJob(',obj.id,')">立即执行</a>',
							    	' &nbsp; <a class="glyphicon text-success" href="',webroot,'/ttsTaskJobLog/f-view/manager.shtml?projectid=${param.projectid}&jobid=',obj.id,'">调度日志</a>');
							return ['<tr>',
							    	'<td>',obj.id,'</td>',
							    	'<td>',obj.name,'</td>',
		...
		status : function(id, status) {
			...
			if(confirm('您确定要'+_statusname+'该任务吗?')) {
				JUtil.ajax({
					url : '${webroot}/ttsTaskJob/f-json/updateStatus.shtml',
		...
		execJob : function(id) {
			if(confirm('您确定要立即执行该任务吗?')) {
				JUtil.ajax({
					url : '${webroot}/ttsTaskJob/f-json/execJob.shtml',
					data : { id: id },
					success : function(json) {
						if (json.code === 0) {
							message('执行成功');
		...
		del : function(id) {
			if(confirm('您确定要删除该任务吗?')) {
				JUtil.ajax({
					url : '${webroot}/ttsTaskJob/f-json/delete.shtml',
					data : { id: id },
					success : function(json) {
						if (json.code === 0) {
							message('删除成功');
